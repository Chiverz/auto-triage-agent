...
def assistant_is_done(message):
    done_phrases = ["no further questions", "that's all I need", "thank you for the answers"]
...
def show_initial_form():
    ...
    with st.form(key="initial_form"):
        ...
        reason_for_change = st.text_area(
            "Reason for Change?",
            value="""Uninhabitable locations require a separate order journey when compared to the current standard ordering journey we have built.
TalkTalk require the ability to be able to place SOGEA and FTTP orders to Uninhabitable sites via SOM2.""",
        )
        vision_objectives = st.text_area(
            "The Vision & Objectives",
            value="Allow Partners to place orders for Uninhabitable locations such as Traffic lights, oyster card terminals etc.",
        )
        risks_challenges = st.text_area(
            "Known Risks & Challenges",
            value="""We used to order WLR to uninhabitable premises however, WLR stop sell came into affect on the 5th September.
As we haven’t built this into SOGEA and FTTP joruneys, we don’t have a product available to sell which could result in losing business.""",
        ...
        delivery_date = st.text_area(
            "Aspirational Delivery Date and why",
            value="Ideally, we need this by September 5th. As this isn’t achievable, early next year would be a good target.",
        )
        benefits_justifications = st.text_area(
            "Non-Quantifiable Benefits & Justifications",
            value="""Partner satisfaction
Reduce risk of partner churn
Continue to sell to uninhabitable premises after WLR closure""",
        )
        measure_benefits = st.text_area(
            "How are you going to measure the benefits?", value="Uninhabitable orders placed."
        )
        ...
        st.session_state.messages = [
            {
                "role": "system",
                "content": (
                    "You are an expert business analyst working in the telecoms "
                    "industry. The business stakeholder has provided you with the "
                    "1-pager business demand. Provide what you believe to be the "
                    "highest priority follow-up question to ask the business "
                    "stakeholder, until you have no further questions — and give "
                    "the 'why'."
                ),
            }
        ] + st.session_state.one_pager
...
def show_chat_interface():
    ...
    st.markdown("---")
    if len([m for m in st.session_state.messages if m["role"] == "assistant"]) >= 2 and st.button(
        "✅ Finish Q&A and Generate Requirements"
    ):
